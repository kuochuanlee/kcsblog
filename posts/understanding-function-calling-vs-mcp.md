---
title: 釐清 function calling 與 MCP 的區別
...
LLM →（function call 訊號）→ Agent → 直接在 Agent 內部呼叫程式碼（in-process）
...
Agent →（MCP 協議） → 連到外部 MCP Server（out-of-process）
...
在實際運作中，當你使用一個支援 MCP 的應用程式時，Agent 會把 MCP Server 提供的工具描述塞進 prompt 給 LLM，LLM 透過 Function Calling 訊號告知 Agent 要呼叫哪個工具，Agent 再透過 MCP 協議去 MCP Server 實際執行。

```
LLM  →（function call 訊號）→  Agent  →（MCP 協議）→  MCP Server
```
