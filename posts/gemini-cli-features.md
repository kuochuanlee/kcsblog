...
| 元件 | 一句話定義 |
|------|-----------|
| **Memory** | 記憶區，要求 AI 必須記住的背景知識或工作守則  |
| **Tool** | 一支程式，AI 主動調用來做一件固定邏輯的事 |
| **MCP Server** | 一組相關 Tool 的集合 + 對外連接通道，讓AI獲得外部能力可調用Tool |
| **Skill** | 自然語言寫的專業知識，AI 讀了改變其判斷方式，變成專家 |
| **Hook** | 我寫的攔截規則，當 AI 準備執行某個動作時，在執行前或執行後會自動觸發，用來過濾、修改或記錄行為 |
| **Command** | 我自己定義的快捷指令，裡面可以內含固定的 prompt、執行特定指令或程式，用 `/指令名稱` 觸發，把常用操作變成一鍵執行 |
| **Extension** | 大禮包，把處理某類事情所需的 Tool + MCP Server + Skill + Commands + Hooks 全部打包，一次安裝搞定 |
| **Policy** | 預先寫死的行為邊界規則，定義哪些動作永遠允許、永遠禁止、需要詢問，AI 不能自己跳過，系統層級硬性約束 |
| **Privacy** | 一個開關，控制我跟 AI 的對話內容和資料，有沒有權限被 Google 拿去做模型訓練或其他用途 |